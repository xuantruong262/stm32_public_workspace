...
//__attribute__((section(".RAM_D1"), aligned(4)))
//volatile uint16_t FrameBuffB[240][320];
__attribute__((section(".RAM_D1"), aligned(4)))
...
//	memcpy(FrameBuff,FrameBuffB,sizeof(FrameBuff));
	//uint16_t *byte_fr565 = (uint16_t*) FrameBuff;
//	for (uint32_t i = 0; i < height; i++) {
//		for(uint32_t j = 0; j < width; j++){
//			FrameBuff[i][j] = (FrameBuff[i][j] >> 8) | (FrameBuff[i][j] << 8);
//		}
//	}
	if (num_trunk > 1) {
		SizePerTrunk = total_byte / num_trunk;
		...
		asm volatile("" ::: "memory");
#endif
        HAL_SPI_Transmit_DMA(hSPI, (uint8_t*)FrameBuff, SizePerTrunk );
	} else {
        SizePerTrunk = total_byte;
#ifdef USING_CACHE
        SCB_CleanDCache_by_Addr((uint32_t*) data_frame, ALIGN32(SizePerTrunk));
		...
		asm volatile("" ::: "memory");
#endif
        HAL_SPI_Transmit(hSPI, (uint8_t*) FrameBuff, SizePerTrunk,HAL_MAX_DELAY);
		HAL_GPIO_WritePin(TFT_CS_GPIO_Port, TFT_CS_Pin, GPIO_PIN_SET);
	}
//	uint16_t pix_data = 0;
//	for (uint32_t i = 0; i < height*width; i++) {
//		pix_data = data_frame[i];
//		//pix_data = (pix_data >> 8) | (pix_data << 8);
//		HAL_SPI_Transmit(hSPI, &pix_data, 2,HAL_MAX_DELAY);
//	}
//	HAL_GPIO_WritePin(TFT_CS_GPIO_Port, TFT_CS_Pin, GPIO_PIN_SET);
}
