...
{
  HAL_SD_Card_SDMMCStatusTypeDef CardStatus;
  uint32_t speedgrade;
  uint32_t unitsize;
  uint32_t tickstart;
  ...
  while ((HAL_SD_GetCardState(hsd) != HAL_SD_Card_SDMMC_TRANSFER))
  {
...
/**
  ...
  *         the CID register.
  * @param  hsd: Pointer to SD handle
  * @param  pCID: Pointer to a HAL_SD_Card_SDMMCCIDTypeDef structure that
...
/**
  ...
  *         the CSD register.
  * @param  hsd: Pointer to SD handle
  * @param  pCSD: Pointer to a HAL_SD_Card_SDMMCCSDTypeDef structure that
...
/**
  ...
  * @param  hsd: Pointer to SD handle
  * @param  pStatus: Pointer to the HAL_SD_Card_SDMMCStatusTypeDef structure that
...
/**
  ...
  * @param  hsd: Pointer to SD handle
  * @param  pCardInfo: Pointer to the HAL_SD_Card_SDMMCInfoTypeDef structure that
  *         will contain the SD card status information
  * @retval HAL status
  */
...
HAL_SD_Card_SDMMCStateTypeDef HAL_SD_GetCardState(SD_HandleTypeDef *hsd)
{
  ...
  return (HAL_SD_Card_SDMMCStateTypeDef)cardstate;
}
...
{
  HAL_SD_Card_SDMMCStateTypeDef CardState;
  ...
  if ((CardState == HAL_SD_Card_SDMMC_RECEIVING) || (CardState == HAL_SD_Card_SDMMC_SENDING))
  {
    hsd->ErrorCode = SDMMC_CmdStopTransfer(hsd->Instance);
  }
...
static uint32_t SD_InitCard(SD_HandleTypeDef *hsd)
{
  HAL_SD_Card_SDMMCCSDTypeDef CSD;
  uint32_t errorstate;
  uint16_t sd_rca = 0U;
  uint32_t tickstart = HAL_GetTick();
