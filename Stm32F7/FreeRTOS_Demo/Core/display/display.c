...
uint8_t LCD_DisplayBMP(const char *file_name) {
	...
	br = 0;
	int i = 0;
	uint32_t num_sector = (SizeOfFrameBuff + 512) / 512;
	int tmp_framsize = 0;
	uint32_t R_byte = 0;
	if (bmp.bpp == 2) { // RGB565
		Offset = 0x42;
		...
		while ((int) ((total_pix * bmp.bpp) - SizeOfFrameBuff * i) > 0) {
			tmp_framsize = SizeOfFrameBuff;
			for(uint32_t y = 0; y < num_sector;y++){
				R_byte = (tmp_framsize > 512) ? 512 : tmp_framsize;
				f_read(&file, Frame_buf + y*512, R_byte, &br);
				tmp_framsize = tmp_framsize - R_byte;
			}
		...
		while ((int) ((total_pix * bmp.bpp) - SizeOfFrameBuff * i) > 0) {
			...
			Display_BMPImage(pos_x, pos_y, bmp.width, w_per_chunk, Frame_buf, 3);
			pos_y += w_per_chunk;
			i++;
		}
	} else {
