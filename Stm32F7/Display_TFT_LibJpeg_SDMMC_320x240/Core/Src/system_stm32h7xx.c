 ...
 * @retval None
 */
extern uint32_t _sitcm_code;       // Bắt đầu ITCM RAM
extern uint32_t _eitcm_code;       // Kết thúc ITCM RAM
extern uint32_t _sitcm_code_load;  // Vị trí trong FLASH

static void ITCM_Copy(void) {
	uint32_t *src = &_sitcm_code_load;
	uint32_t *dst = &_sitcm_code;
	while (dst < &_eitcm_code) {
		*dst++ = *src++;
	}
}
void SystemInit(void) {
 ...
 __IO uint32_t tmpreg;
  ...
  SCB->VTOR = VECT_TAB_BASE_ADDRESS | VECT_TAB_OFFSET; /* Vector Table Relocation in Internal D1 AXI-RAM or in Internal FLASH */
	 ...
	 SCB->ITCMCR = (1 << 0);   // enable ITCM (bit EN)
	 ITCM_Copy();
}
