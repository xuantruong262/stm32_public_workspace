...
void ST7735_Draw_PixData(uint8_t pos_x, uint8_t pos_y, uint8_t width, uint8_t height, uint16_t *data_frame)
{
    ST7735_SendCommand(0x2A);                             // Column addr set
    ...
    // HAL_SPI_Transmit(&hspi2, data_frame, width * height * 2, HAL_MAX_DELAY);  
  ...
  uint16_t d;
    for (int i = 0; i < width * height; i++)
    {
        // R_565 = (data_frame[i]) & 0x1F;
        // G_565 = (data_frame[i] >> 5) & 0x3F;
        // B_565 = (data_frame[i] >> 11) & 0x1F;
        // data_frame[i] = ( B_565 << 11) | (G_565 << 5) | R_565;

        //RGB_565 = (RGB_565 >> 8) | (RGB_565 << 8);
        data_frame[i] = (data_frame[i] >> 8) | (data_frame[i] << 8);
        HAL_SPI_Transmit(&hspi2, &data_frame[i], 2,HAL_MAX_DELAY);
        // hi = (data_frame[i]) >> 8;    
        // lo = (data_frame[i]);
